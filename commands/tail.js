...
export class TailCommand {
  ...
  async execute(args) {
    ...
    for (const input of files) {
      ...
      if (!stat) { out.push(`tail: ${input}: No such file or directory`); continue; }
      if (stat.isDirectory) { out.push(`tail: ${input}: Is a directory`); continue; }
      const analysis = this.policy.analyze({ path, mime: stat.mime_type, size: stat.size });
      if (!analysis.isText) {
        out.push(`tail: ${input}: binary file — ${this.policy.suggestForBlock(analysis)}`);
        continue;
      }
      try {
        const content = await this.terminal.fs.getContent(path) || '';
        const lines = content.split('\n');
        const tailLines = lines.slice(Math.max(0, lines.length - count));
        out.push(tailLines.join('\r\n'));
      } catch (e) {
        out.push(`tail: ${input}: ${e.message}`);
      }
    }
    return out.join('\r\n\r\n') + '\r\n';
  }
  ...
  parseArgs(args) {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (a === '-n' || a === '--lines') {
        ...
        if (!isNaN(n) && n >= 0) { count = n; i++; } else { return { count, files, error: 'tail: invalid line count' }; }
      } else {
        files.push(a);
      }
    }
